...
def handle_message(message_text, sender_id):
    ...
    if '你好' in message_text or '請問' in message_text or '嗨' in message_text or '哈囉' in message_text or 'hi' in message_text or 'hello' in message_text:
        if len(message_text ) < 10:
            return '你好！🙂\n請問我能為您做些什麼？ '
    # Email
    if '信' in message_text or 'e-mail' in message_text or 'e mail' in message_text or 'email' in message_text or 'mail' in message_text :
        ...
        if '沒收到' in message_text or '沒有收到' in message_text or '垃圾信' in message_text :
    ...
    if '電腦' in message_text or '教室' in message_text or '中心' in message_text :
        if '開' in message_text or '用' in message_text or '借' in message_text :
            return '您好🙂  電腦教室相關訊息請參考 http://cc.ncku.edu.tw/files/11-1255-3303.php?Lang=zh-tw ，謝謝。'

    #dorm
    if '宿' in message_text :
        ...
        if '不通' in message_text or '不能' in message_text or '斷' in message_text or '認證' in message_text or '連' in message_text or \
           '無法' in message_text or '問題' in message_text:
            faq = template_json.Template_json(sender_id,template_type=2,
                   text="是否曾申請過帳號呢? (請用是/否按扭回答以便記錄)", payload_yes = "START_STATE_YES", payload_no = "START_STATE_NO" )
            return faq
